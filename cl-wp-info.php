...
define( 'CL_WP_INFO_VERSION', '1.4.30' );
define( 'CL_WP_INFO_MIN_PHP', '7.4' );
define( 'CL_WP_INFO_MIN_DB', '5.5.5' );
define( 'CL_WP_INFO_MIN_DB_MARIA', '10' );

define( 'CL_WP_INFO_REC_PHP', '8.1' );
define( 'CL_WP_INFO_REC_MYSQL', '8.0' );
define( 'CL_WP_INFO_REC_MARIA', '10.5' );

$cl_wp_info_menu_hook    = '';
$cl_wp_info_submenu_hook = '';
...
function cl_wp_info_add_menu_page() {
	global $cl_wp_info_menu_hook, $cl_wp_info_submenu_hook;

	$cl_wp_info_menu_hook = add_menu_page(
	...
	$cl_wp_info_submenu_hook = add_submenu_page(
		'cl-wp-info',
...
function cl_wp_info_load_custom_wp_admin_style( $hook ) {
	...
	if ( $cl_wp_info_menu_hook !== $hook && $cl_wp_info_submenu_hook !== $hook ) {
		return;
	}

	if ( $cl_wp_info_submenu_hook === $hook ) { // Javascript only in submenu page Tools.
		wp_enqueue_script( 'cl_wp_info_tools_admin_js', plugins_url( 'js/cl-wp-info-tools.min.js', __FILE__ ), array(), CL_WP_INFO_VERSION, true );
	}

	wp_enqueue_style( 'cl_wp_info_admin_css', plugins_url( 'css/cl-wp-info-admin.min.css', __FILE__ ), array(), CL_WP_INFO_VERSION );
}
...
function cl_wp_info_init() {
	load_plugin_textdomain( 'cl-wp-info' );
}
...
function cl_wp_info_general() {
	global $cl_wp_info_obj;
	?>

	<div class="cl-info-made-by"><?php $cl_wp_info_obj->cl_wp_info_made_by(); ?></div>

	<div class="cl-info-general"><?php $cl_wp_info_obj->cl_wp_info_general(); ?></div>
